...
{
    public class ProductionQCSampleService : IProductionQCSampleService
    {
        private readonly IUnitOfWork _uow;
        public ProductionQCSampleService(IUnitOfWork uow)
        {
            _uow = uow;
        }

        public async Task<bool> AddProductionQCSample(AddProductionQCSampleDTO request, string userId, CancellationToken ct)
        {
            var sample = request.ToDomain(userId);
            await _uow.production_qc_sample.AddAsync(sample, userId, ct);
            ...
            if (request.InspectionObjects != null && request.InspectionObjects.Any())
            {
                foreach (var inspectionObject in request.InspectionObjects)
                {
                    ...
                    await _uow.production_qc_sample_result.AddAsync(sampleResult, userId, ct);

                    var logResult = sampleResult.ToDomain(userId, sample.IsSamplePassed);
                    await _uow.log_production_qc_sample_result.AddAsync(logResult, userId, ct);
                }
            }
        ...
        public async Task<List<ResponseProductionQCSample>> ListAllProductionQCSamplesByQcId(string qcId, CancellationToken ct, int lastCount, int skipRecords)
        {
            var query = _uow.production_qc_sample.GetQueryable();

            var result = await query.Data
                .Where(x => x.QcId == qcId)
                .Include(x => x.Production_QC_Sample_Results)
        ...
        public async Task<List<ResponseProductionQCSample>> GetProductionQCSampleById(CancellationToken ct, string productionQCSampleId)
        {
            var query = _uow.production_qc_sample.GetQueryable();
            var result = await query.Data
                                    .Include(x => x.Production_QC_Sample_Results)
                                    .OrderByDescending(x => x.CreatedDate)
                                    .Where(x => x.Id == productionQCSampleId)
                                    .ToListAsync(ct);
            ArgumentFalseException.ThrowIfFalse(result.Any(), "No records found");
        ...
        public async Task<bool> UpdateProductionQCSample(UpdateProductionQCSampleDTO request, string userId, CancellationToken ct)
        {
            // Fetch the Sample
            var query = _uow.production_qc_sample.GetQueryable();
            var result = await query.Data.Include(x => x.Production_QC_Sample_Results).Where(x => x.Id == request.Id).SingleOrDefaultAsync(ct) ?? throw new ArgumentFalseException("No active record found to update");
            ...
            await _uow.production_qc_sample.UpdateAsync(result, userId, ct);

            foreach (var inspectionObject in request.InspectionObjects)
            {
                // Fetch the Sample Results
                var resultObject = result.Production_QC_Sample_Results.FirstOrDefault(x =>
                ...
                await _uow.log_production_qc_sample_result.AddAsync(logResultObject, userId, ct);
                ...
                await _uow.production_qc_sample_result.UpdateAsync(resultObject, userId, ct);
            }
            await _uow.CommitAsync();
            return true;
        }
        ...
        public async Task<bool> IsProductionQcIdAvailable(string? Id, CancellationToken ct)
        {
            var result = await _uow.production_qc.AnyAsync(ct, x => x.Id == Id);
            return result.Data;
        }
