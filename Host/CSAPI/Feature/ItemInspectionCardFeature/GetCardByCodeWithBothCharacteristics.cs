...
{
    public class GetCardByCodeWithBothCharacteristics : IItemInspectionCardFeature
    {
        public static void Map(IEndpointRouteBuilder app) => app
            .MapGet($"/{nameof(GetCardByCodeWithBothCharacteristics)}", Handle)
            .WithSummary("Get Card By Code With Both Qualitative and Quantitative Characteristics")
            .Produces(200)
            .Produces(404)
            .Produces(500)
        ...
        private static async Task<IResult> Handle([FromQuery] string itemCode, ICustomLogger logger, IItemInspectionCardService svc, CancellationToken ct)
        {
            ...
            try
            {
                var result = await svc.GetCardByCodeWithBothCharacteristics(itemCode, ct);
