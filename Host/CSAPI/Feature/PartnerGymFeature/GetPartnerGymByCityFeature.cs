...
{
    public class GetPartnerGymByCity : IPartnerGymFeature
    {
        public static void Map(IEndpointRouteBuilder app) => app
            .MapGet($"/{nameof(GetPartnerGymByCity)}", Handle)
            .WithSummary("Get partner gyms by city")
            ...
            CancellationToken ct)
        {
            try
            {
                var gyms = await svc.GetPartnerGymByCityRaw(city, ct);
                if (gyms.Count == 0)
