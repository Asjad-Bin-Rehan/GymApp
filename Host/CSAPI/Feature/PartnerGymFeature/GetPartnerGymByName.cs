...
{
    public class GetPartnerGymByName : IPartnerGymFeature
    {
        public static void Map(IEndpointRouteBuilder app) => app
            .MapGet($"/{nameof(GetPartnerGymByName)}", Handle)
            .WithSummary("Get partner gyms by name")
            ...
            CancellationToken ct)
        {
            try
            {
                var gyms = await svc.GetPartnerGymByNameRaw(name, ct);
                if (gyms.Count == 0)
