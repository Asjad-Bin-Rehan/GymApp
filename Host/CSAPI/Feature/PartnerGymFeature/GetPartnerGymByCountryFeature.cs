...
{
    public class GetPartnerGymByCountry : IPartnerGymFeature
    {
        public static void Map(IEndpointRouteBuilder app) => app
            .MapGet($"/{nameof(GetPartnerGymByCountry)}", Handle)
            .WithSummary("Get partner gyms by country")
            ...
            CancellationToken ct)
        {
            try
            {
                var gyms = await svc.GetPartnerGymByCountryRaw(country, ct);
                if (gyms.Count == 0)
