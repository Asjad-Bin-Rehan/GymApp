...
{
    public class GetOrganization : IOrganizationFeature
    {
        public static void Map(IEndpointRouteBuilder app) => app
            .MapPost($"/{nameof(GetOrganization)}", Handle)
            .WithSummary("Get an Organization")
            .Produces<GetOrganizationResponse>(HTTPStatusCode200.Ok)
            .Produces(HTTPStatusCode400.NotFound)
            .Produces(HTTPStatusCode500.InternalServerError)
            ;

        private static async Task<IResult> Handle([FromQuery] string? organizationId, [FromQuery] string? tenantId, [FromQuery] string? name, [FromQuery] int lastCount, [FromQuery] int skipRecords, IOrganizationService svc, IUserContext user, ICustomLogger logger, CancellationToken ct)
        {
            try
            {
                var result = await svc.GetOrganization(organizationId, tenantId, name, lastCount, skipRecords, ct);
