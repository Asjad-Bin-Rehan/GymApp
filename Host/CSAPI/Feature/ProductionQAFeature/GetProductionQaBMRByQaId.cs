...
{
    public class GetProductionQaBMRByQaId : IProductionQAFeature
    {
        public static void Map(IEndpointRouteBuilder app) => app
            .MapGet($"/{nameof(GetProductionQaBMRByQaId)}", Handle)
            .WithSummary("Get BMR No by QA ID")
            .Produces(200)
            .Produces(404)
            .Produces(500)
            .Produces<string?>();
        ...
        private static async Task<IResult> Handle([FromQuery] string qaId, ICustomLogger logger, IProductionQAService svc, CancellationToken ct)
        {
            ...
            try
            {
                var result = await svc.GetProductionQaBMRByQaId(qaId, ct);
