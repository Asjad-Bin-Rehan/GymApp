...
{
    public class AddProductionQC : IProductionQCFeature
    {
        public static void Map(IEndpointRouteBuilder app) => app
            .MapPost($"/{nameof(AddProductionQC)}", Handle)
            .WithSummary("Add a Production QC record to DB")
            .Produces(201)
            .Produces(402)
            .Produces(500)
            .WithRequestValidation<AddProductionQCDTO>()
            .Produces<string>();

        public class RequestValidator : AbstractValidator<AddProductionQCDTO>
        {
        ...
        private static async Task<IResult> Handle([FromBody] AddProductionQCDTO req, IUserContext user, ICustomLogger logger, IProductionQCService svc, CancellationToken cancellationToken)
        {
            int statusCode = HTTPStatusCode200.Created;
            string message = "Success";
            try
            {
                var result = await svc.AddProductionQC(req, user.Data.UserId ?? "Anonymous", cancellationToken);
